...
pgClient.connect()
const jeunes = await fetchJeunesInstanceIdNayantPasReponduAUneCampagneActive(pgClient)
pgClient.end()
...
async function loadData(bigqueryClient, datasetId, tableId, file) {
    ...
    const bigQueryJob = await bigqueryClient.dataset(datasetId)
        .table(tableId)
        .load(file, options)
...
function getBigQueryClient() {
    return new BigQuery({credentials: JSON.parse(process.env.GOOGLE_APPLICATION_CREDENTIALS)})
}
