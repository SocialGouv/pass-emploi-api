...
describe('GetMonSuiviPoleEmploiQueryHandler', () => {
  ...
  describe('handle', () => {
    ...
    it("renvoie message d'erreur quand les démarches sont KO", async () => {
      getDemarchesQueryGetter.handle.resolves(
      ...
      expect(
        isSuccess(result) && result.data.queryModel.messageDonneesManquantes
      ).to.equal('Les Démarches sont temporairement indisponibles')
      expect(
        isSuccess(result) && result.data.queryModel.rendezVous
      ).to.deep.equal([rdv])
      expect(
