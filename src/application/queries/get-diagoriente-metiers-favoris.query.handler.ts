...
> {
  ...
  async handle(
    ...
    const metiersFavoris: DiagorienteMetiersFavorisQueryModel = {
      aDesMetiersFavoris: Boolean(
        result.data.data.userByPartner.favorites.length
      ),
      metiersFavoris: result.data.data.userByPartner.favorites
        .filter(favori => favori.favorited)
        .map(favori => ({ rome: favori.tag.code, titre: favori.tag.title }))
    }
    return success(metiersFavoris)
  }
